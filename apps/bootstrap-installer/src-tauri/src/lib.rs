//! RuyiHermesAgent Setup — Tauri entrypoint.
//!
...
pub fn run() {
    ...
    // RuyiHermesAgent is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "RuyiHermesAgent installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // RuyiHermesAgent is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "RuyiHermesAgent" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "RuyiHermesAgent" shortcuts that install.ps1 creates, and a
                ...
                if bootstrap::hermes_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            ...
                            tracing::info!(
                                "RuyiHermesAgent already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .expect("error while running RuyiHermesAgent Setup");
}
